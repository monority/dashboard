...
- Name: api-mocking.spec.ts >> Task Management with API Mocking >> should retry failed requests with mock
- Location: e2e\api-mocking.spec.ts:106:3
...
```ts
  ...
  52  |     });
  53  |
  54  |     await page.goto('/task');
  55  |     await page.waitForLoadState('networkidle');
  56  |
  57  |     // Find create button
  58  |     const createButton = page.locator('button:has-text("Create"), button:has-text("Add")').first();
  59  |
  60  |     // If button exists, test creation flow
  61  |     if (await createButton.isVisible()) {
  62  |       await createButton.click();
  63  |
  64  |       // Look for form
  65  |       const titleInput = page.locator('input[placeholder*="title" i]').first();
  66  |
  67  |       if (await titleInput.isVisible()) {
